...
const PosInEbarimtConfigCard = ({
  config,
  configKey,
  onSave,
  onDelete,
}: {
  config: TPosInEbarimtConfig;
  configKey: string;
  ...
  const handleSubmit = (data: TPosInEbarimtConfig) => {
...
export const PosInEBarimtConfigForm = () => {
  ...
  const deleteConfig = async (configKey: string) => {
    ...
    const existingConfig = getConfigByPosId(config.posId);

    if (existingConfig) {
      await removePosInEbarimtConfig(existingConfig._id);
    }
  ...
  const saveConfig = async (configKey: string, configData: any) => {
    ...
    const existingConfig = getConfigByPosId(configData.posId);

    if (existingConfig) {
      await savePosInEbarimtConfig(configData, 'update', existingConfig._id);
    } else if (configData.posId) {
      await savePosInEbarimtConfig(configData, 'create');
    }
  };
...
export default PosInEBarimtConfigForm;
