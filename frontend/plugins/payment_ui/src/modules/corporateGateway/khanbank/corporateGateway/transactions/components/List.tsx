...
const List = ({ statement, queryParams, loading, showLatest }: Props) => {
  ...
  const totalCount = statement?.total?.count || 0;
  ...
  const transactions = useMemo(() => {
    ...
    switch (type) {
      case 'income':
        return statement.transactions.filter((t) => t.amount > 0);
      case 'outcome':
        return statement.transactions.filter((t) => t.amount < 0);
      default:
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted border-b">
              <tr className="text-left">
                ...
                <th className="px-4 py-3">Begin Balance</th>
                <th className="px-4 py-3">End Balance</th>
                <th className="px-4 py-3">Amount</th>
                <th className="px-4 py-3">Related Account</th>
              </tr>
            </thead>
